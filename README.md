...
This injects an **allow-all base underneath your config**, so every tool auto-approves
*unless you explicitly `deny` it* — your `deny` rules still win. In the TUI it shows a
red warning and requires you to accept the risk before it takes effect (the prompt is
skipped when there is no TTY, so automation keeps working).
